...
class Request
  DOCUMENT_ROOT = '/Library/WebServer/Documents/'.freeze
  PATH_TO_CSS = '/sieve/css/exam.css'.freeze
  PATH_TO_IMAGES = '/mewb7/illustrations/fullsize/'.freeze
  PATH_TO_XML = 'questions/MEWB7.xml'.freeze
  PATH_TO_MARKDOWN_XSLT = 'xsl/fmp2md.xsl'.freeze
  ...
  def initialize(params = CGI.new.params)
    @params = params
    @format = params['format'].first.to_sym if params.has_key? 'format'
